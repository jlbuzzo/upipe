	...
	while ( <IN1> ) {
		...
		if ( $print ) {
			...
			if ( scalar(@reps_inside) >= 2 ) {
				...
				open (OUT1,">$pd/$fn.$RAND.temp_rep_el");
				foreach my $rep ( @reps_inside ) {
					print OUT1 "$rep->[0]\t$rep->[1]\t$rep->[2]\t$rep->[3]\n";
				}
				close(OUT1);
				`mergeBed -i $pd/$fn.$RAND.temp_rep_el > $pd/$fn.$RAND.temp_rep_el.merged`;
				`rm $pd/$fn.$RAND.temp_rep_el`;
				open (IN2,"< $pd/$fn.$RAND.temp_rep_el.merged");
				my @buffer = ();
				while( <IN2> ) {
					my @tokens = split ( /[ \t\cI]+/,$_ );
					push(@buffer,\@tokens);
				...
				close(IN2);
				`rm $pd/$fn.$RAND.temp_rep_el.merged`;
